...
impl Graph {
    pub fn count_paths(&self) -> usize {
        self.count_paths_inner(&START.to_string(),&mut vec![])
    }

    fn count_paths_inner(&self, from: &String, stack: &mut Vec<String>) -> usize {
        let mut paths = 0;
        ...
        for connected in node.edges.iter() {
            if connected == END {
                ...
                paths += match (stack.contains(connected), is_upper(connected)) {
                    (false, _)    => self.count_paths_inner(connected, stack),
                    (true, true)  => self.count_paths_inner(connected, stack),
                    (true, false) => 0,
                }
            }
        }
